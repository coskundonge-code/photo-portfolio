...
import { ImageIcon, FolderOpen, ShoppingBag, Users, ArrowRight, Loader2 } from 'lucide-react';
import { getPhotos, getProjects, getProducts, getMembers } from '@/lib/supabase';
...
export default function AdminPage() {
  ...
  const [stats, setStats] = useState({ photos: 0, projects: 0, products: 0, members: 0 });
  ...
  const loadStats = async () => {
    try {
      const [photos, projects, products, members] = await Promise.all([
        getPhotos(),
        getProjects(),
        getProducts(),
        getMembers()
  ...
  const cards = [
    ...
    { label: 'Üyeler', value: stats.members, icon: Users, href: '/admin/members', color: 'bg-orange-500' },
  ];
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
                ...
                <div className="space-y-2">
                  ...
                  <Link href="/admin/members" className="block px-4 py-3 bg-neutral-800 rounded-lg text-neutral-300 hover:bg-neutral-700 hover:text-white transition-colors">
                    👥 Üyeleri Yönet
                  </Link>
                </div>
              </div>
