...
import { Loader2, Trash2, ArrowLeft, Check, CreditCard, Truck, Shield } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const shipping = subtotal > 2000 ? 0 : 150; // 2000₺ üstü ücretsiz kargo
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      if (order) {
        ...
        // E-mail gönder (backend API gerekli)
        // await sendOrderEmail(order.id, customerInfo.email);
      }
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center px-4">
        <div className="max-w-md text-center">
          ...
          <Link
            href="/shop"
            className="inline-block px-8 py-3 bg-black text-white rounded hover:bg-neutral-800 transition-colors"
          >
            Alışverişe Devam Et
          </Link>
        </div>
      ...
      <header className="bg-white border-b border-neutral-200">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className={`flex items-center gap-2 ${step === 'cart' ? 'text-black' : 'text-neutral-400'}`}>
              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step === 'cart' ? 'bg-black text-white' : 'bg-neutral-200'}`}>1</span>
              <span className="hidden sm:inline">Sepet</span>
            </div>
            <div className="w-8 h-px bg-neutral-300" />
            <div className={`flex items-center gap-2 ${step === 'info' ? 'text-black' : 'text-neutral-400'}`}>
              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step === 'info' ? 'bg-black text-white' : 'bg-neutral-200'}`}>2</span>
            ...
            <div className={`flex items-center gap-2 ${step === 'payment' ? 'text-black' : 'text-neutral-400'}`}>
              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step === 'payment' ? 'bg-black text-white' : 'bg-neutral-200'}`}>3</span>
          ...
          <div className="lg:col-span-2">
            ...
            {step === 'cart' && (
              <div className="bg-white rounded-lg p-6">
                <h2 className="text-xl font-medium mb-6">Sepetiniz ({cart.length} ürün)</h2>
                
                <div className="space-y-4">
                  {cart.map((item, index) => (
                    <div key={index} className="flex gap-4 p-4 border border-neutral-200 rounded-lg">
                      {/* Ürün görseli */}
                      <div className="relative w-24 h-24 bg-neutral-100 rounded overflow-hidden flex-shrink-0">
                        <Image
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-neutral-500 mt-1">
                          {item.size.name} ({item.size.dimensions}) • {item.frame.name} Çerçeve • {item.style === 'mat' ? 'Mat' : 'Full Bleed'}
                        </p>
                ...
                <button
                  ...
                  className="w-full mt-6 py-4 bg-black text-white font-medium rounded hover:bg-neutral-800 transition-colors"
                >
                  Devam Et
                </button>
              </div>
            ...
            {step === 'info' && (
              <div className="bg-white rounded-lg p-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border border-neutral-300 rounded focus:ring-2 focus:ring-black focus:border-transparent outline-none"
                ...
                <h2 className="text-xl font-medium mt-8 mb-6">Teslimat Adresi</h2>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-neutral-700 mb-1">İl</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-neutral-700 mb-1">Posta Kodu</label>
                      <input
                        type="text"
                ...
                <div className="flex gap-4 mt-6">
                  ...
                  <button
                    ...
                    className="flex-1 py-3 bg-black text-white font-medium rounded hover:bg-neutral-800 transition-colors disabled:bg-neutral-300 disabled:cursor-not-allowed"
                  >
                    Ödemeye Geç
                  </button>
                </div>
            ...
            {step === 'payment' && (
              <div className="bg-white rounded-lg p-6">
                <h2 className="text-xl font-medium mb-6">Ödeme</h2>
                
                {/* Ödeme yöntemi seçimi */}
                <div className="space-y-3 mb-8">
                  <label className="flex items-center gap-4 p-4 border border-neutral-200 rounded-lg cursor-pointer hover:border-neutral-300 transition-colors">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-neutral-500">Visa, Mastercard, Troy</p>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center gap-4 p-4 border border-neutral-200 rounded-lg cursor-pointer hover:border-neutral-300 transition-colors">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-neutral-500">+50₺ kapıda ödeme ücreti</p>
                    </div>
                  </label>
                </div>
                ...
                <div className="space-y-4 mb-8">
                  <div>
                    <label className="block text-sm font-medium text-neutral-700 mb-1">Kart Numarası</label>
                    <input
                      type="text"
                      className="w-full px-4 py-3 border border-neutral-300 rounded focus:ring-2 focus:ring-black focus:border-transparent outline-none"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-neutral-700 mb-1">Son Kullanma</label>
                      <input
                        type="text"
                        className="w-full px-4 py-3 border border-neutral-300 rounded focus:ring-2 focus:ring-black focus:border-transparent outline-none"
                        placeholder="AA/YY"
                ...
                <div className="flex gap-4">
                  ...
                  <button
                    ...
                    disabled={submitting}
                    className="flex-1 py-3 bg-black text-white font-medium rounded hover:bg-neutral-800 transition-colors disabled:bg-neutral-400"
                  >
                    {submitting ? (
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg p-6 sticky top-8">
              ...
              <div className="mt-6 pt-4 border-t border-neutral-200 space-y-2 text-xs text-neutral-500">
                <p>✓ Ücretsiz kargo (₺2.000 üstü siparişlerde)</p>
                <p>✓ 30 gün içinde koşulsuz iade</p>
                <p>✓ Güvenli ödeme</p>
              </div>
            </div>
          </div>
